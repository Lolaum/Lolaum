  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        ...
        {routineType === "english_book" && currentBooks.length > 0 && (
          <p className="text-xs text-gray-400 bg-gray-50 rounded-lg px-3 py-2 mb-3">
            원서 읽기는 스크린샷으로 인증해 독서 기록 세부 내용은 표시되지
            않습니다.
          </p>
        )}
            ...
            {books
              ...
              .map((book) => (
                ...
                >
                  ...
                  <span className="text-[10px] text-gray-600 max-w-[60px] truncate">
                    {book.title}
                  </span>
                </button>
              ))}
...
function MorningInsightView({
  ...
  const conditionColor =
    data.avgCondition >= 80
      ? "#10b981"
      : data.avgCondition >= 60
        ? "#eab32e"
        : "#f97316";
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          { label: "학습 표현", value: `${data.totalExpressions}개`, color },
          {
            ...
            color: routineType === "english" ? "#7dd3fc" : "#a78bfa",
          },
        ].map((s) => (
          <div
            key={s.label}
...
function FinanceInsightView({
  ...
  const necessaryPercent =
    currentMonth.total > 0
      ? Math.round((currentMonth.necessary / currentMonth.total) * 100)
      : 0;
  const emotionalPercent =
    currentMonth.total > 0
      ? Math.round((currentMonth.emotional / currentMonth.total) * 100)
      : 0;
  const valuePercent =
    currentMonth.total > 0
            ...
            <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-500 mb-4">
              ...
              <span>
                ...
                <span className="font-semibold" style={{ color: "#8b5cf6" }}>
                  {currentMonth.value.toLocaleString()}원
                </span>
              </span>
            </div>
